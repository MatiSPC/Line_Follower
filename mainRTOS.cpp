...
SemaphoreHandle_t xStartStopSemaphore;
...
class DrivingFunctions : public MotorController{
  ...
  : MotorController(leftConfig),
    LeftMotor(leftConfig), 
    RightMotor(rightConfig){}
...
void DriveTask(void *pvParameters) {
  ...
  for (;;) {
      if (xSemaphoreTake(xStartStopSemaphore, portMAX_DELAY)) {
          while (!ButtonPressed(PIND3)) {
              if (CheckDrivingCondition(LeftSensor) && CheckDrivingCondition(RightSensor)) {
                  Vehicle.DriveForward();
              } else if (!CheckDrivingCondition(LeftSensor)) {
                  Vehicle.TurnRight();
              } else if (!CheckDrivingCondition(RightSensor)) {
                  Vehicle.TurnLeft();
              } else {
                  Vehicle.StopVehicle();
              }
...
void ButtonTask(void *pvParameters) {
  while (1) {
      if (ButtonPressed(PIND3)) {
          xSemaphoreGive(xStartStopSemaphore);
          while (ButtonPressed(PIND3)); 
      }
      vTaskDelay(pdMS_TO_TICKS(100));
  }
...
int main(void)
{
  ...
  xStartStopSemaphore = xSemaphoreCreateBinary();
